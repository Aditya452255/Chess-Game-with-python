...
# Minimal Flask app: serve the new front-end-only index and assets.
...
def index():
    ...
    # allow importing from web/src or project src
    ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
    ...
    def serialize_board(board):
        ...
        # Prefer web/src for importing game logic
        ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
        WEB_SRC = os.path.join(os.path.dirname(__file__), 'src')
        if os.path.exists(WEB_SRC):
            SRC = WEB_SRC
        ...
        # Simple in-memory game
        GAME = {'board': Board(), 'next_player': 'white'}
